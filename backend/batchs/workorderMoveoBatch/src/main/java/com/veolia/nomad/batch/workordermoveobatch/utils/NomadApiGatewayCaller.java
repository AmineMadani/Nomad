...
public class NomadApiGatewayCaller extends AmazonWebServiceClient {
    private static final String API_GATEWAY_SERVICE_NAME = "execute-api";

    private final AWSCredentialsProvider credentials;
    private final AWS4Signer signer;

    private final JsonResponseHandler<NomadApiGatewayResponse> responseHandler;
    private final JsonErrorResponseHandler errorResponseHandler;

    public NomadApiGatewayCaller(String accessKey, String secretAccessKey, String apiKey, String region, URI endpoint) {
        ...
        final Unmarshaller<NomadApiGatewayResponse, JsonUnmarshallerContext> responseUnmarshaller = in -> new NomadApiGatewayResponse(in.getHttpResponse());
        this.responseHandler = SdkStructuredPlainJsonFactory.SDK_JSON_FACTORY.createResponseHandler(metadata, responseUnmarshaller);

        JsonErrorUnmarshaller defaultErrorUnmarshaller = new JsonErrorUnmarshaller(NomadApiGatewayException.class, null) {
            @Override
            public AmazonServiceException unmarshall(JsonNode jsonContent) throws Exception {
                return new NomadApiGatewayException(jsonContent.toString());
            }
        };
	...
	public NomadApiGatewayResponse execute(HttpMethodName method, String resourcePath, InputStream content) {
