...
public class BatchConfiguration {
    ...
    ItemReader<WorkOrder> reader(DataSource dataSource) throws Exception {
        ...
        sqlPagingQueryProviderFactoryBean.setSelectClause(
                "SELECT "
                + "	case when wts.wts_code='ANNULE' then 'S' else null end as codeMvt, "
                + "	'NOMAD' as origin, "
                + "	ast.ast_code as type, "
                + "	wtr.wtr_code as reason, "
                + "	wts.wts_code as status, "
                + "	case when  "
                + "			(SElECT cr.libelle "
                + "			FROM asset.config_rue cr  "
                + "			WHERE ST_DWithin(cr.geom, w.geom, 100) "
                + "			order by ST_Distance(cr.geom, w.geom) "
                + "			limit 1) is not null "
                + "		then  "
                + "			(SElECT cr.libelle "
                + "			FROM asset.config_rue cr  "
