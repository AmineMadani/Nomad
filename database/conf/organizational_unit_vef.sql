...
select distinct  trim(SUBSTRING(tmp.territoire, 1, POSITION(' ' IN tmp.territoire))) AS territoire_code,
       ...
       (select out.id from organizational_unit_type out where out.out_code = 'TERRITORY') as out_id,
       (select org.id 
...
(select distinct  (select ctr.id  FROM contract ctr where ctr.ctr_code = tmp.code_contrat) as ass_ctr_id ,
       (select org.id from organizational_unit org where out_id = (select out.id from organizational_unit_type out where out.out_code = 'TERRITORY')  
