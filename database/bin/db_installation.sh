...
function install_all() {
    ...
    SQL_SOURCES="$ROOT_DIR/sql/drop_schemas.sql \
               ...
               $ROOT_DIR/sql/1_wo_mgnt_drop_tables.sql \"
               $ROOT_DIR/sql/2_wo_mgnt_create_tables.sql \"
               $ROOT_DIR/sql/3_wo_mgnt_create_fk.sql "
...
psql -v ON_ERROR_STOP=1 -v srid=$SRID $CONNINFO < $LAYER_WO_PARAMS_FILE  >/dev/null || failed "Impossible de charger les paramètres de gestino des interventions '$LAYER_WO_PARAMS_FILE' !"
