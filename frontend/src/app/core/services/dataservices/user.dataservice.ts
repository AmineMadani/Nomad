...
export class UserDataService {
  ...
  async getUserInformation(): Promise<Observable<User>> {
    const res = await lastValueFrom(
      this.http
        .get<User[]>(`${this.configurationService.apiUrl}user/information`)
        .pipe(
          timeout(2000),
          catchError(async () => {
  ...
  public updateUser(user: User): void {
    ...
    this.http
      .put(`${this.configurationService.apiUrl}user/update`, usr)
      .subscribe({
  ...
  getAllUserAccount(): Observable<User[]> {
    return this.http.get<User[]>(
      `${this.configurationService.apiUrl}user/all-account`
    );
  }
}
