...
export class MobileHomeActionsComponent implements OnInit {
  ...
  public async onGenerateWorkOrder(){
    ...
    let l_ctr_id = await firstValueFrom(this.referentialService.getReferentialIdByLongitudeLatitude('contract', centerMapPosition.lng.toString(), centerMapPosition.lat.toString()));
    let l_cty_id = await firstValueFrom(this.referentialService.getReferentialIdByLongitudeLatitude('city', centerMapPosition.lng.toString(), centerMapPosition.lat.toString()));
    let params:any = {};
